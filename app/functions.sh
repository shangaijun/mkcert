...
function get_traefik_container {
    # First try to get the docker-gen container ID from the container label.
    local docker_gen_cid="$(labeled_cid mkcert.services.traefik)"

    # If the labeled_cid function dit not return anything and the env var is set, use it.
    if [[ -z "$docker_gen_cid" ]] && [[ -n "${TRAEFIK_CONTAINER:-}" ]]; then
        docker_gen_cid="$TRAEFIK_CONTAINER"
    fi
...
function get_nginx_proxy_container {
    ...
    local nginx_cid="$(labeled_cid mkcert.services.nginx)"
...
function reload_nginx {
    ...
    local _traefik_container=$(get_traefik_container)
    local _nginx_proxy_container=$(get_nginx_proxy_container)

    if [[ -n "${_traefik_container:-}" ]]; then
        # Using docker-gen and nginx in separate container
        echo "Reloading traefik (using separate container ${_traefik_container})..."
        for file in $(ls /traefik); do touch "/traefik/$i"; done; 
    fi

    if [[ -n "${_docker_gen_container:-}" ]]; then
        # Using docker-gen and nginx in separate container
        echo "Reloading nginx docker-gen (using separate container ${_docker_gen_container})..."
        docker_restart "${_docker_gen_container}"
    fi
